...
def user_specified_client2(roa, rov, roa_rov, neither, numClients, select_algo, csvfile):
    ...
    while croa < roa or crov < rov or croa_rov < roa_rov or cneither < neither:
        ...
        c.ipaddress = resultIP
        c.prefix = IPaddressPrefix
        if check_roa(c.AS.ASN) and check_rov(c.AS.ASN):
            c.roaCovered = True
            c.rovCovered  = True
            if croa_rov < roa_rov:
                resultClientList.append(c)
                croa_rov += 1
        elif check_roa(c.AS.ASN):
            c.roaCovered = True
            if croa < roa:
                resultClientList.append(c)
                croa += 1
...
def get_highCoverage_distribution(repeatTime):
    ...
    while repeatTime > 0:
        print('run')
        clients = get_user_wASN(1000, countries, cweights, 'matching')
        roa = 0
        rov = 0
        roa_rov = 0
        neither = 0
        for c in clients:
            if check_roa(c.AS.ASN) and check_rov(c.AS.ASN):
                ...
                neither += 1
        print('roa:', roa, 'rov:', rov, 'roa_rov', roa_rov, 'neither', neither)
        totalRoa += roa
        totalRov += rov
        totalRoaRov += roa_rov
    ...
    print('roa:', round(totalRoa/totalRuns), 'rov:', round(totalRov/totalRuns), 'roa_rov', round(totalRoaRov/totalRuns), 'neither', round(totalNeither/totalRuns))

def analyzeClientPickle(file):
