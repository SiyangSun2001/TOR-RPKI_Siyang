...
def adl_cov(num_relays, v4_covered, top_num_nc, asn4s):
    """
    graph the projection of ROA coverage if top TOR relay AS would cover their whole IP space
    use processed pickle as input file in the main function
    """
    # Top ASNs stacked bar for number of relays
    d = dict()      # covered
    dd = dict()     # not covered
...
def main(): 
    """
    call the above graphing function at the end of main function to 
    graph the projections of ROA coverage under different scenarios.
    """
    with open("processed/2020-09-13-00-processed.pickle", 'rb') as f1:
        relays = pickle.load(f1)
        sept = get_data(relays)
    with open("processed/2020-09-14-00-processed.pickle", 'rb') as f2:
        relays = pickle.load(f2)
        octo = get_data(relays)
    with open("processed/2020-09-15-00-processed.pickle", 'rb') as f3:
        relays = pickle.load(f3)
        nov = get_data(relays)
    with open("processed/2020-09-16-00-processed.pickle", 'rb') as f4:
        relays = pickle.load(f4)
        dec = get_data(relays)
    with open("processed/2020-09-17-00-processed.pickle", 'rb') as f5:
        relays = pickle.load(f5)
        jan = get_data(relays)
    with open("processed/2020-09-18-00-processed.pickle", 'rb') as f6:
        relays = pickle.load(f6)
        feb = get_data(relays)
    with open("processed/2020-09-22-00-processed.pickle", 'rb') as f7:
